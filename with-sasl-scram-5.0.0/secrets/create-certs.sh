...
# Generate CA key
openssl req -new -x509 -keyout cacert.key -out cacert.crt -days $VALIDITY -subj '/CN=ca1.test.confluent.io/OU=TEST/O=CONFLUENT/L=PaloAlto/S=Ca/C=US'
...
do
  ...
  openssl x509 -req -CA cacert.crt -CAkey cacert.key -in kafka.$i.csr -out kafka.$i.crt -days $VALIDITY -CAcreateserial -passin pass:confluent
